	...
	try {
		if(msg.toString().startsWith("#login ")) {
			if(client.getInfo("loginID") != null) {
				client.sendToClient("ERRROR - Already logged in.");
				client.close();
				return;
			}
			
			String[] messageParts = msg.toString().split(" ");
			if(messageParts.length < 2) {
				client.sendToClient("No login provided");
				client.close();
				return;
			}
			...
			System.out.println("Message received: #login " + id + " from null.");
		...
		if(client.getInfo("loginID") == null) {
			client.sendToClient("ERRROR - Not logged in.");
			client.close();
			return;
		}
